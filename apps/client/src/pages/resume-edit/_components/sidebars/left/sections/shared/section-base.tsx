...
import { defaultSections, SectionItem, SectionKey, SectionWithItem } from "@reactive-resume/schema";
    ...
    >
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-x-4">
          ...
          <h2 className="text-2xl font-bold line-clamp-1">{section.name}</h2>
        </div>
      ...
      <main className={cn("grid transition-opacity", !section.visible && "opacity-50")}>
        {section.items.length === 0 && (
          <Button
            variant="outline"
            onClick={onCreate}
            className="py-6 leading-relaxed border-dashed gap-x-2 hover:bg-secondary-accent"
          >
            <Plus size={14} />
