  ...
  return (
    ...
    >
      <div className="flex items-center gap-x-2">
        ...
        <p className="flex-1 text-xs font-medium truncate">{name}</p>
      </div>
    </div>
  );
};
      ...
      <main className="grid gap-y-4">
        ...
        >
          {layout.map((page, pageIndex) => {
            ...
            return (
              <div key={pageIndex} className="p-3 pb-4 border rounded">
                ...
                <div className="grid items-start grid-cols-2 gap-x-4">
                  <Column id={mainIndex} name={t`主体`} items={main} />
                  <Column id={sidebarIndex} name={t`侧边栏`} items={sidebar} />
                </div>
              </div>
            );
          })}
        ...
        <Button variant="outline" className="ml-auto" onClick={onAddPage}>
          <Plus />
          <span className="ml-2">{t`添加一页`}</span>
        </Button>
      </main>
    </section>
  );
